...
void RCconfig_macrlc(void)
{
  ...
  if ( MacRLC_ParamList.numelt > 0) {
	...
	RC.mac[j]->max_ul_rb_index);
      ...
      // Lấy chuỗi config ra một biến cho code gọn gàng dễ đọc
      ...
      if (strcmp(sched_mode_str, "default") == 0) {
        ...
        RC.mac[j]->scheduler_mode = SCHED_MODE_DEFAULT;
        ...
        RC.mac[j]->scheduler_mode = SCHED_MODE_FAIR_RR;
        printf("sched mode = fairRR %d [%s]\n", global_scheduler_mode, sched_mode_str);
      } 
      // ++++++++++ THÊM CÁC SCHEDULER CỦA NHÓM VÀO ĐÂY ++++++++++
      else if (strcmp(sched_mode_str, "max_ci") == 0) {
        global_scheduler_mode = SCHED_MODE_MAX_CI;
        RC.mac[j]->scheduler_mode = SCHED_MODE_MAX_CI;
        printf("sched mode = max_ci %d [%s]\n", global_scheduler_mode, sched_mode_str);
      } 
      else if (strcmp(sched_mode_str, "pf") == 0) {
        global_scheduler_mode = SCHED_MODE_PF;
        RC.mac[j]->scheduler_mode = SCHED_MODE_PF;
        printf("sched mode = pf %d [%s]\n", global_scheduler_mode, sched_mode_str);
      } 
      else if (strcmp(sched_mode_str, "ai") == 0) {
        global_scheduler_mode = SCHED_MODE_AI;
        RC.mac[j]->scheduler_mode = SCHED_MODE_AI;
        printf("sched mode = ai %d [%s]\n", global_scheduler_mode, sched_mode_str);
      } 
      // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      else {
        // Nếu gõ sai tên trong file .conf thì fallback về default cho an toàn
        global_scheduler_mode = SCHED_MODE_DEFAULT;
        RC.mac[j]->scheduler_mode = SCHED_MODE_DEFAULT;
        printf("sched mode = default (fallback) %d [%s]\n", global_scheduler_mode, sched_mode_str);
      }
