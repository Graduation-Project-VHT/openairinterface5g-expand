...
//------------------------------------------------------------------------------
void schedule_dlsch(module_id_t module_idP, frame_t frameP, sub_frame_t subframeP, int *mbsfn_flag)
{
...
void getRepetition(UE_TEMPLATE *pue_template, unsigned int *maxRep, unsigned int *narrowBandindex)
{
  ...
  AssertFatal(
      pue_template->physicalConfigDedicated->ext4->epdcch_Config_r11->config_r11.choice.setup.setConfigToAddModList_r11->list.count
          > 0,
      "epdcch config list is empty");
  epdcch_setconfig_r11 = pue_template->physicalConfigDedicated->ext4->epdcch_Config_r11->config_r11.choice.setup
                             .setConfigToAddModList_r11->list.array[0];
  AssertFatal(epdcch_setconfig_r11->ext2 != NULL && epdcch_setconfig_r11->ext2->mpdcch_config_r13 != NULL,
              " mpdcch config not found");
 ...
 * Schedule the DLSCH
 */
void schedule_ue_spec(module_id_t module_idP, int CC_id, frame_t frameP, sub_frame_t subframeP)
//------------------------------------------------------------------------------
{
  ...
  for (int UE_id = UE_info->list.head; UE_id >= 0; UE_id = UE_info->list.next[UE_id]) {
    ...
    if (ue_sched_ctrl->pre_nb_available_rbs[CC_id] == 0) { // no RBs allocated
      LOG_D(MAC, "%d.%d: no RBs allocated for UE %d on CC_id %d\n", frameP, subframeP, UE_id, CC_id);
      continue;
    }

    int dci_dl_pdu_idx = ue_sched_ctrl->pre_dci_dl_pdu_idx;
    if (dci_dl_pdu_idx < 0) {
      dci_dl_pdu_idx = CCE_try_allocate_dlsch(module_idP, CC_id, subframeP, UE_id, ue_sched_ctrl->dl_cqi[CC_id]);
      if (dci_dl_pdu_idx < 0) {
    ...
    if (dci->pdu_type != NFAPI_DL_CONFIG_DCI_DL_PDU_TYPE || dci->dci_dl_pdu.dci_dl_pdu_rel8.rnti != rnti
        || dlsch->pdu_type != NFAPI_DL_CONFIG_DLSCH_PDU_TYPE || dlsch->dlsch_pdu.dlsch_pdu_rel8.rnti != rnti) {
      LOG_E(MAC, "illegal dl_config_pdu_list index %d for UE %d/RNTI %04x\n", dci_dl_pdu_idx, UE_id, rnti);
      continue;
    }

    const int harq_pid = frame_subframe2_dl_harq_pid(cc->tdd_Config, frameP, subframeP);
    ...
    if (round_DL != 8) {
      ...
      if (ue_sched_ctrl->cdrx_configured) {
         ...
         */
        if (harq_pid == 0) {
          VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME(VCD_SIGNAL_DUMPER_VARIABLES_DRX_RETRANSMISSION_HARQ0,
                                                  (unsigned long)ue_sched_ctrl->drx_retransmission_timer[0]);
        }
      }
      ...
      for (int i = 0; i < ue_sched_ctrl->dl_lc_num; ++i) {
        ...
        const uint32_t data = min(ue_sched_ctrl->dl_lc_bytes[i], TBS - ta_len - header_length_total - sdu_length_total - 3);
        LOG_D(MAC,
        ...
        LOG_T(MAC, "[eNB %d][%d] CC_id %d Got %d bytes :", module_idP, lcid, CC_id, sdu_lengths[num_sdus]);
      ...
      if (ta_len + sdu_length_total + header_length_total > 0) {
        ...
        if (ta_update != 31) {
          LOG_D(MAC,
                "[eNB %d][DLSCH] Frame %d Generate header for UE_id %d on CC_id %d: sdu_length_total %d, num_sdus %d, "
                "sdu_lengths[0] %d, sdu_lcids[0] %d => payload offset %d,timing advance value : %d, padding %d,post_padding "
                "%d,(mcs %d, TBS %d, nb_rb %d),header_length %d\n",
                module_idP,
                frameP,
                UE_id,
                CC_id,
        ...
        T(T_ENB_MAC_UE_DL_PDU_WITH_DATA,
          ...
          T_INT(subframeP),
          T_INT(harq_pid),
          T_BUFFER(dlsch_pdu->payload[0], TBS));
        ...
        if (ue_sched_ctrl->cdrx_configured) {
          ...
          VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME(VCD_SIGNAL_DUMPER_VARIABLES_DRX_INACTIVITY,
                                                  (unsigned long)ue_sched_ctrl->drx_inactivity_timer);
 ...
 */
void schedule_ue_spec_br(module_id_t module_idP, frame_t frameP, sub_frame_t subframeP)
//------------------------------------------------------------------------------
{
  int CC_id = 0;
  int UE_id = -1;
  ...
  if (cc[CC_id].radioResourceConfigCommon_BR) {
    ...
    switch (prach_ParametersListCE_r13->list.count) {
      ...
      case 2:
        ...
        AssertFatal(ext4_pucch->pucch_NumRepetitionCE_Msg4_Level1_r13 != NULL,
      ...
      case 1:
        ...
        AssertFatal(ext4_pucch->pucch_NumRepetitionCE_Msg4_Level0_r13 != NULL,
  ...
  for (UE_id = UE_info->list.head; UE_id >= 0; UE_id = UE_info->list.next[UE_id]) {
    ...
    AssertFatal(
        ...
        "NULL\n");
    LTE_EPDCCH_SetConfig_r11_t *epdcch_setconfig_r11 = UE_template->physicalConfigDedicated->ext4->epdcch_Config_r11->config_r11
    ...
    if (subframeP == 5) { // MPDCCH
      ...
      if (round_DL == 8) {
        ...
        for (lcid = NB_RB_MAX - 1; lcid >= DTCH; lcid--) {
          ...
          if (TBS - ta_len - header_len_dcch - sdu_length_total - header_len_dtch > 0) { // NN: > 2 ?
            rlc_status =
                mac_rlc_status_ind(module_idP, rnti, module_idP, frameP, subframeP, ENB_FLAG_YES, MBMS_FLAG_NO, lcid, 0, 0);
        ...
        if ((sdu_length_total + header_len_dcch + header_len_dtch) > 0) {
          ...
          if (header_len_dtch == 0) {
            ...
            header_len_dtch = (header_len_dtch > 0) ? header_len_dtch - header_len_dtch_last
          ...
          if (ta_update != 31) {
            LOG_D(MAC,
                  "[eNB %d][DLSCH] Frame %d Generate header for UE_id %d on CC_id %d: sdu_length_total %d, num_sdus %d, "
                  "sdu_lengths[0] %d, sdu_lcids[0] %d => payload offset %d,timing advance value : %d, padding %d,post_padding "
                  "%d,(mcs %d, TBS %d, nb_rb %d),header_dcch %d, header_dtch %d\n",
                  module_idP,
                  frameP,
                  UE_id,
                  CC_id,
          ...
          int32_t framex10psubframe =
              UE_info->UE_template[CC_id][UE_id].pucch_tpc_tx_frame * 10 + UE_info->UE_template[CC_id][UE_id].pucch_tpc_tx_subframe;
      ...
      if (round_DL < 8) {
        ...
        dl_config_pdu->mpdcch_pdu.mpdcch_pdu_rel13.resource_block_coding =
            getRIV(6, 0, 6) | ((epdcch_setconfig_r11->ext2->mpdcch_config_r13->choice.setup.mpdcch_Narrowband_r13 - 1) << 5);
        ...
        dl_config_pdu->mpdcch_pdu.mpdcch_pdu_rel13.total_dci_length_including_padding =
            0; // this is not needed by OAI L1, but should be filled in
      ...
      LOG_D(MAC,
            ...
            rnti);
      first_rb =
          narrowband_to_first_rb(&cc[CC_id], epdcch_setconfig_r11->ext2->mpdcch_config_r13->choice.setup.mpdcch_Narrowband_r13 - 1);
      dl_config_pdu = &dl_req->dl_config_pdu_list[dl_req->number_pdu];
      memset((void *)dl_config_pdu, 0, sizeof(nfapi_dl_config_request_pdu_t));
      ...
      dl_config_pdu->dlsch_pdu.dlsch_pdu_rel8.resource_block_coding =
          getRIV(to_prb(cc[CC_id].mib->message.dl_Bandwidth), first_rb, 6); // check that this isn't getRIV(6,0,6)
      ...
      ul_config_pdu->uci_harq_pdu.ue_information.ue_information_rel13.total_number_of_repetitions =
          pucchreps[UE_template->rach_resource_type - 1];
      ...
      if (cc[CC_id].tdd_Config == NULL) { // FDD case
        ul_config_pdu->uci_harq_pdu.harq_information.harq_information_rel9_fdd.n_pucch_1_0 =
            n1pucchan[UE_template->rach_resource_type - 1];
...
//------------------------------------------------------------------------------
void update_ul_dci(module_id_t module_idP, uint8_t CC_idP, rnti_t rntiP, uint8_t daiP, sub_frame_t subframe)
//------------------------------------------------------------------------------
{
  eNB_MAC_INST *eNB = RC.mac[module_idP];
  COMMON_channels_t *cc = &eNB->common_channels[CC_idP];
...
//------------------------------------------------------------------------------
void set_ue_dai(sub_frame_t subframeP, int UE_id, uint8_t CC_id, uint8_t tdd_config, UE_info_t *UE_info)
//------------------------------------------------------------------------------
{
  switch (tdd_config) {
    case 0:
    ...
    for (uint16_t i = 0; i < MAX_MOBILES_PER_ENB; i++) {
      ...
      if (frameP % ue_pf_po->T == ue_pf_po->PF_min && subframeP == ue_pf_po->PO) {
        ...
        if (pcch_sdu_length <= TBStable1C[0]) {
          ...
          LOG_E(MAC,
                "[eNB %d] Frame %d : PCCH->PCH CC_id %d, Received %d bytes is over max length(256) \n",
                module_idP,
                frameP,
                CC_id,
                pcch_sdu_length);
        ...
        dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.resource_block_coding =
            getRIV(n_vrb_dl / n_rb_step, first_rb / n_rb_step, Lcrbs / n_rb_step);
        ...
        // paging first_rb log
        LOG_D(MAC,
              "[eNB %d] Frame %d subframe %d PCH: paging_ue_index %d pcch_sdu_length %d mcs %d first_rb %d\n",
              module_idP,
              frameP,
              subframeP,
