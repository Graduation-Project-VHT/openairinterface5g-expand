...
const uint8_t pusch_repetition_Table8_2_36213[3][4] = {{1, 2, 4, 8}, {1, 4, 8, 16}, {1, 4, 16, 32}};
...
//-----------------------------------------------------------------------------
{
  ...
  if (UE_id != -1) {
    ...
    if (sduP != NULL) {
      ...
      if (tmp_snr > 0 && tmp_snr < 63) {
        ...
        if (abs(diff) < snr_thres_tpc) {
          UE_scheduling_control->pusch_cqi_f[CC_idP] =
              ((double)UE_scheduling_control->pusch_cqi_f[CC_idP] * snr_filter_tpc + (double)ul_cqi * (1 - snr_filter_tpc));
    ...
    if (no_sig || sduP == NULL) { // we've got an error on Msg3
      ...
      if (ra->msg3_round >= mac->common_channels[CC_idP].radioResourceConfigCommon->rach_ConfigCommon.maxHARQ_Msg3Tx - 1) {
        ...
        if (ra->rach_resource_type > 0) {
          cancel_ra_proc(enb_mod_idP, CC_idP, frameP, current_rnti); // TODO: Currently we don't support retransmission of Msg3 ( If
                                                                     // in error Cancel RA procedure and reattach)
        } else {
          // first_rb = UE_template_ptr->first_rb_ul[harq_pid]; // UE_id = -1 !!!!
          ra->msg3_round++;
          /* Prepare handling of retransmission */
          get_Msg3allocret(&mac->common_channels[CC_idP], ra->Msg3_subframe, ra->Msg3_frame, &ra->Msg3_frame, &ra->Msg3_subframe);
          // prepare handling of retransmission
    ...
    switch (rx_ces[i]) { // implement and process PHR + CRNTI + BSR
      case POWER_HEADROOM:
        if (UE_id != -1) {
           ...
           * (int8_t)(hundred_times_log10_NPRB[UE_template_ptr->nb_rb_ul[harq_pid] - 1] / 100);i*/
      ...
      case SHORT_BSR:
        ...
        if (UE_id != -1) {
          ...
          UE_template_ptr->estimated_ul_buffer = UE_template_ptr->ul_buffer_info[LCGID0] + UE_template_ptr->ul_buffer_info[LCGID1]
  ...
  for (int i = 0; i < num_sdu; i++) {
    ...
    switch (rx_lcids[i]) {
      ...
      case DCCH1:
        ...
        if ((rx_lengths[i] > DCH_PAYLOAD_SIZE_MAX) || (rx_lengths[i] < 0) || (rx_lengths[i] > (sdu_lenP - (payload_ptr - sduP)))) {
          LOG_E(MAC,
                "[eNB %d/%d] frame %d received DCCH of size %d (too big, maximum allowed is %d, sdu_len %d), dropping packet\n",
                enb_mod_idP,
                CC_idP,
                frameP,
                rx_lengths[i],
        ...
        if (UE_id != -1) {
          if (lcgid_updated[UE_template_ptr->lcgidmap[rx_lcids[i]]] == 0) {
            ...
            UE_template_ptr->estimated_ul_buffer = UE_template_ptr->ul_buffer_info[0] + UE_template_ptr->ul_buffer_info[1]
                                                   + UE_template_ptr->ul_buffer_info[2] + UE_template_ptr->ul_buffer_info[3];
      ...
      default:
        ...
        if (rx_lcids[i] < NB_RB_MAX) {
          if ((rx_lengths[i] > SCH_PAYLOAD_SIZE_MAX) || (rx_lengths[i] < 0)
              || (rx_lengths[i] > (sdu_lenP - (payload_ptr - sduP)))) {
            LOG_E(MAC,
                  "[eNB %d/%d] frame %d received DTCH of size %d (too big, maximum allowed is %d, sdu_len %d), dropping packet\n",
                  enb_mod_idP,
                  CC_idP,
                  frameP,
                  rx_lengths[i],
          ...
          if (UE_id != -1) {
            ...
            if (ue_contextP != NULL) {
              if (ue_contextP->ue_context.DRB_active[rx_lcids[i] - 2] == 0) {
                LOG_E(MAC,
                      "[eNB %d/%d] frame %d received non active DTCH of size %d ( sdu_len %d, lcid %d), dropping packet\n",
                      enb_mod_idP,
                      CC_idP,
                      frameP,
                      rx_lengths[i],
            ...
            /* Adjust buffer occupancy of the correponding logical channel group */
            LOG_D(MAC,
                  "[eNB %d] CC_id %d Frame %d : ULSCH -> UL-DTCH, received %d bytes from UE %d for lcid %d, removing from LCGID "
                  "%ld, %d\n",
                  enb_mod_idP,
                  CC_idP,
                  frameP,
    ...
    if ((sdu_lenP - (payload_ptr - sduP)) >= rx_lengths[i]) {
      ...
      LOG_E(MAC,
            "[eNB %d/%d] frame %d subframe %d rnti %x sdu_len %d  remain_len %d rx_lengths %d\n",
            enb_mod_idP,
            CC_idP,
            frameP,
            subframeP,
            rntiP,
            sdu_lenP,
            (uint16_t)(sdu_lenP - (payload_ptr - sduP)),
            rx_lengths[i]);
      return;
    }
  }
 ...
 */
void schedule_ulsch(module_id_t module_idP, frame_t frameP, sub_frame_t subframeP)
//-----------------------------------------------------------------------------
{
  ...
  for (int CC_id = 0; CC_id < RC.nb_mac_CC[module_idP]; CC_id++, cc++) {
    if (is_prach_subframe0(cc->tdd_Config != NULL ? cc->tdd_Config->subframeAssignment : 0,
                           cc->tdd_Config != NULL ? 1 : 0,
...
void schedule_ulsch_rnti(module_id_t module_idP, int CC_id, frame_t frameP, sub_frame_t subframeP, unsigned char sched_subframeP)
{
  ...
  for (int UE_id = UE_info->list.head; UE_id >= 0; UE_id = UE_info->list.next[UE_id]) {
    ...
    if (dci_ul_pdu_idx < 0) {
      dci_ul_pdu_idx = CCE_try_allocate_ulsch(module_idP, CC_id, subframeP, UE_id, UE_sched_ctrl_ptr->dl_cqi[CC_id]);
      if (dci_ul_pdu_idx < 0) {
    ...
    if (hi_dci0_pdu->pdu_type != NFAPI_HI_DCI0_DCI_PDU_TYPE || hi_dci0_pdu->dci_pdu.dci_pdu_rel8.rnti != rnti) {
      LOG_E(MAC, "illegal hi_dci0_pdu_list index %d for UE %d/RNTI %04x\n", dci_ul_pdu_idx, UE_id, rnti);
      continue;
    }
     ...
     */
    const int32_t fx10psf = (UE_template_ptr->pusch_tpc_tx_frame * 10) + UE_template_ptr->pusch_tpc_tx_subframe;
    uint32_t tpc = 1;
    if (((fx10psf + 10) <= (frameP * 10 + subframeP)) // normal case
        || ((fx10psf > (frameP * 10 + subframeP)) && (((10240 - fx10psf + frameP * 10 + subframeP) >= 10)))) { // frame wrap-around
    ...
    if (round_index == 0) {
      ...
      uint32_t cqi_req = 0;
      LOG_D(MAC, "RRC Connection status %d, cqi_timer %d\n", status, UE_sched_ctrl_ptr->cqi_req_timer);
      ...
      if (UE_sched_ctrl_ptr->cdrx_configured) {
        ...
        VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME(VCD_SIGNAL_DUMPER_VARIABLES_DRX_INACTIVITY,
      ...
      if (dlsch_flag == 1) {
        if (cqi_req == 1) {
          ...
          ul_req_tmp_body->ul_config_pdu_list[ul_req_index]
              .ulsch_cqi_harq_ri_pdu.initial_transmission_parameters.initial_transmission_parameters_rel8.tl.tag =
              NFAPI_UL_CONFIG_REQUEST_INITIAL_TRANSMISSION_PARAMETERS_REL8_TAG;
          ul_req_tmp_body->ul_config_pdu_list[ul_req_index]
              .ulsch_cqi_harq_ri_pdu.initial_transmission_parameters.initial_transmission_parameters_rel8.n_srs_initial =
              0; // last symbol not punctured
          ul_req_tmp_body->ul_config_pdu_list[ul_req_index]
              .ulsch_cqi_harq_ri_pdu.initial_transmission_parameters.initial_transmission_parameters_rel8
          ...
          ul_req_tmp_body->ul_config_pdu_list[ul_req_index]
              .ulsch_harq_pdu.initial_transmission_parameters.initial_transmission_parameters_rel8.tl.tag =
              NFAPI_UL_CONFIG_REQUEST_INITIAL_TRANSMISSION_PARAMETERS_REL8_TAG;
          ul_req_tmp_body->ul_config_pdu_list[ul_req_index]
              .ulsch_harq_pdu.initial_transmission_parameters.initial_transmission_parameters_rel8.n_srs_initial =
              0; // last symbol not punctured
          ul_req_tmp_body->ul_config_pdu_list[ul_req_index]
              .ulsch_harq_pdu.initial_transmission_parameters.initial_transmission_parameters_rel8
              .initial_number_of_resource_blocks = rb_table[rb_table_index];
        }
      ...
      if (first_rb != UE_template_ptr->first_rb_ul[harq_pid] || nb_rb != UE_template_ptr->nb_rb_ul[harq_pid])
        LOG_D(MAC,
      ...
      if (dlsch_flag == 1) {
        if (cqi_req == 1) {
          ...
          ul_req_tmp_body->ul_config_pdu_list[ul_req_index]
              .ulsch_cqi_harq_ri_pdu.initial_transmission_parameters.initial_transmission_parameters_rel8
              .initial_number_of_resource_blocks = UE_template_ptr->nb_rb_ul[harq_pid];
        } else {
          ul_req_tmp_body->ul_config_pdu_list[ul_req_index].pdu_type = NFAPI_UL_CONFIG_ULSCH_HARQ_PDU_TYPE;
          ulsch_harq_information = &ul_req_tmp_body->ul_config_pdu_list[ul_req_index].ulsch_harq_pdu.harq_information;
          ul_req_tmp_body->ul_config_pdu_list[ul_req_index]
...
//-----------------------------------------------------------------------------
{
  ...
  for (UE_id = UE_info->list.head; UE_id >= 0; UE_id = UE_info->list.next[UE_id]) {
    ...
    if (UE_template->configured == false) {
      LOG_D(MAC,
            "[eNB %d] frame %d subframe %d, UE %d: not configured, skipping UE scheduling \n",
            module_idP,
            frameP,
            subframeP,
            UE_id);
    ...
    for (n = 0; n < UE_info->numactiveULCCs[UE_id]; n++) {
      ...
      Total_Num_Rep_ULSCH =
          pusch_repetition_Table8_2_36213[pusch_maxNumRepetitionCEmodeA_r13][UE_template->pusch_repetition_levels];
      UL_Scheduling_DCI_SF =
          (40 - 4 - Total_Num_Rep_ULSCH) % 10; // TODO: [eMTC Scheduler] To be removed after scheduler relaxation task
      UL_Scheduling_DCI_Frame_Even_Odd_Flag =
          !(((40 - 4 - Total_Num_Rep_ULSCH) / 10) % 2); // TODO: [eMTC Scheduler] To be removed after scheduler relaxation task

      /* If frameP odd don't schedule */
      if ((frameP & 1)
          == UL_Scheduling_DCI_Frame_Even_Odd_Flag) { // TODO: [eMTC Scheduler] To be removed after scheduler relaxation task
        ...
        if ((UE_template->ul_SR > 0 || round_UL > 0 || status < RRC_CONNECTED) && (subframeP == UL_Scheduling_DCI_SF)) {
           ...
           */
          LOG_D(MAC,
                "[eNB %d][PUSCH %d] Frame %d subframe %d Scheduling UE %d/%x in round_UL %d(SR %d,UL_inactivity timer "
                "%d,UL_failure timer %d,cqi_req_timer %d)\n",
                module_idP,
                harq_pid,
                frameP,
                subframeP,
          ...
          if (round_UL == 0) {
            ...
            AssertFatal(UE_template->physicalConfigDedicated->ext4->epdcch_Config_r11->config_r11.present
            ...
            AssertFatal(
                UE_template->physicalConfigDedicated->ext4->epdcch_Config_r11->config_r11.choice.setup.setConfigToAddModList_r11
                    != NULL,
                "UE_template->physicalConfigDedicated->ext4->epdcch_Config_r11->config_r11.choice.setup.setConfigToAddModList_r11 "
                "= NULL\n");
            ...
            AssertFatal(epdcch_setconfig_r11->ext2->mpdcch_config_r13->present
            ...
            AssertFatal(epdcch_setconfig_r11->ext2->numberPRB_Pairs_v1310->present
            ...
            LOG_D(MAC,
                  "[PUSCH %d] Frame %d, Subframe %d: Adding UL 6-0A MPDCCH for BL/CE UE %d/%x, ulsch_frame %d, ulsch_subframe %d, "
                  "UESS MPDCCH Narrowband %d\n",
                  harq_pid,
                  frameP,
                  subframeP,
                  UE_id,
            ...
            UE_template->first_rb_ul[harq_pid] =
                narrowband_to_first_rb(cc, epdcch_setconfig_r11->ext2->mpdcch_config_r13->choice.setup.mpdcch_Narrowband_r13 - 1);
            ...
            hi_dci0_pdu->mpdcch_dci_pdu.mpdcch_dci_pdu_rel13.mpdcch_narrowband =
            ...
            hi_dci0_pdu->mpdcch_dci_pdu.mpdcch_dci_pdu_rel13.mpdcch_transmission_type =
                epdcch_setconfig_r11->transmissionType_r11; // distibuted
            AssertFatal(
                UE_template->physicalConfigDedicated->ext4->epdcch_Config_r11->config_r11.choice.setup.startSymbol_r11 != NULL,
                "UE_template->physicalConfigDedicated->ext4->epdcch_Config_r11->config_r11.choice.setup.startSymbol_r11 is null\n");
            hi_dci0_pdu->mpdcch_dci_pdu.mpdcch_dci_pdu_rel13.start_symbol =
                *UE_template->physicalConfigDedicated->ext4->epdcch_Config_r11->config_r11.choice.setup.startSymbol_r11;
            hi_dci0_pdu->mpdcch_dci_pdu.mpdcch_dci_pdu_rel13.ecce_index = 0; // Note: this should be dynamic
            hi_dci0_pdu->mpdcch_dci_pdu.mpdcch_dci_pdu_rel13.aggreagation_level =
                24; // OK for CEModeA r1-3 (9.1.5-1b) or CEModeB r1-4
            ...
            hi_dci0_pdu->mpdcch_dci_pdu.mpdcch_dci_pdu_rel13.ce_mode =
                (UE_template->rach_resource_type < 3) ? 1 : 2; // already set above...
            hi_dci0_pdu->mpdcch_dci_pdu.mpdcch_dci_pdu_rel13.drms_scrambling_init =
                epdcch_setconfig_r11->dmrs_ScramblingSequenceInt_r11;
            ...
            hi_dci0_pdu->mpdcch_dci_pdu.mpdcch_dci_pdu_rel13.frequency_hopping_flag =
                1 - epdcch_setconfig_r11->ext2->mpdcch_config_r13->choice.setup.mpdcch_pdsch_HoppingConfig_r13;
            ...
            LOG_D(MAC,
                  "[PUSCH %d] Frame %d, Subframe %d: Adding UL CONFIG. Request for BL/CE UE %d/%x, ulsch_frame %d, ulsch_subframe "
                  "%d, UESS mpdcch narrowband %d\n",
                  harq_pid,
                  frameP,
                  subframeP,
                  UE_id,
            ...
            AssertFatal(epdcch_setconfig_r11->ext2->numberPRB_Pairs_v1310->present
                        ...
                        "epdcch_setconfig_r11->ext2->numberPRB_Pairs_v1310->present is not setup\n");
            LOG_D(MAC,
                  "[PUSCH %d] Frame %d, Subframe %d: Adding UL 6-0A MPDCCH for BL/CE UE %d/%x, ulsch_frame %d, ulsch_subframe "
                  "%d,UESS MPDCCH Narrowband %d\n",
                  harq_pid,
                  frameP,
                  subframeP,
  ...
  for (int i = 0; i < ul_req_tmp->number_of_pdus; i++) {
    ...
    if (ul_config_pdu->ulsch_pdu.ulsch_pdu_rel13.repetition_number
      ...
      ul_config_pdu_Rep->ulsch_pdu.ulsch_pdu_rel13.repetition_number =
